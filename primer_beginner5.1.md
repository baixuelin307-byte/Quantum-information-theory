5.1 Communicating a Trit Using a Qubit

1. 任务目标

Alice 手里有一个 trit：

$a\in{0,1,2}$

她希望把这个信息发送给 Bob。

如果只允许发送一个 classical bit，那么 Alice 只能发送：

$0$

或者：

$1$

但是 trit 有三个可能值，因此一个 bit 不可能把三个值全部完美区分。

2. Classical Bit Strategy

一个简单的 classical bit strategy 可以是：

trit 0 → 发送 bit 0

trit 1 → 发送 bit 1

trit 2 → 也发送 bit 1

Bob 收到：

bit 0 → 猜 trit 0

bit 1 → 猜 trit 1

这样三个输入的成功率分别是：

$1,\ 1,\ 0$

所以：

$P_{\text{worst}}=0$

也就是说，某一个输入会被完全牺牲。

3. Randomized Bit Strategy

为了避免某一个 trit 永远失败，可以加入随机策略。

通过适当随机化，可以让三个输入的 worst-case success probability 都达到：

$\frac{1}{2}$

因此 classical bit strategy 的最优 worst-case performance 是：

$P_{\text{worst}}^{\text{bit}}=\frac{1}{2}=50%$

这是后面和 qubit strategy 比较的基准。

4. Qubit Strategy

Alice 可以把三个 trit 编码成三个不同的 qubit states：

$0\rightarrow|\phi_0\rangle$

$1\rightarrow|\phi_1\rangle$

$2\rightarrow|\phi_2\rangle$

这三个状态构成对称的 trine states。

它们在二维空间中大致呈现 120° 对称结构。

            |φ1>
           /
          /
         O------> |φ0>
          \
           \
            |φ2>

由于 qubit 是二维系统，不可能存在三个两两正交的状态，因此：

一个 qubit 不能完美编码并区分一个 trit。

但是，可以通过合适的 measurement，让判断成功率高于 classical bit。

5. Trine States 的重要性质

三个对称状态满足：

\frac{3}{2}I$

这个 $\frac{3}{2}I$ 不是人为规定的，而是把三个具体的 trine state projection matrices 相加以后得到的结果。

例如取：

$|\psi_0\rangle=(1,0)$

$|\psi_1\rangle=(-\frac12,\frac{\sqrt3}{2})$

$|\psi_2\rangle=(-\frac12,-\frac{\sqrt3}{2})$

那么：

$|\psi_0\rangle\langle\psi_0|

\begin{pmatrix}
1&0\
0&0
\end{pmatrix}$

$|\psi_1\rangle\langle\psi_1|

\begin{pmatrix}
\frac14&-\frac{\sqrt3}{4}\
-\frac{\sqrt3}{4}&\frac34
\end{pmatrix}$

$|\psi_2\rangle\langle\psi_2|

\begin{pmatrix}
\frac14&\frac{\sqrt3}{4}\
\frac{\sqrt3}{4}&\frac34
\end{pmatrix}$

三个矩阵相加后，非对角项互相抵消：

$-\frac{\sqrt3}{4}+\frac{\sqrt3}{4}=0$

最后得到：

\frac32 I
$

所以：

\frac32 I$

6. 为什么会出现 $\frac{2}{3}$？

一个合法的 POVM measurement 需要满足：

$E_0+E_1+E_2=I$

但是刚才三个 projection 相加得到：

$\frac32 I$

所以需要乘一个归一化系数：

$\frac23$

因为：

$\frac23\times\frac32=1$

因此定义：

$E_i=\frac23|\psi_i\rangle\langle\psi_i|$

于是：

$\sum_iE_i

\frac23
\sum_i|\psi_i\rangle\langle\psi_i|$

代入：

I$

所以这里的 $\frac23$ 本质上是一个 normalization factor。

7. 为什么成功率可以达到 $\frac{2}{3}$？

如果 Alice 发送：

$|\psi_i\rangle$

Bob 正确得到 measurement outcome $i$ 的概率是：

$P(i|i)=\langle\psi_i|E_i|\psi_i\rangle$

代入：

$E_i=\frac23|\psi_i\rangle\langle\psi_i|$

得到：

$P(i|i)

\frac23
\langle\psi_i|\psi_i\rangle
\langle\psi_i|\psi_i\rangle$

由于：

$\langle\psi_i|\psi_i\rangle=1$

所以：

$P(i|i)=\frac23$

因此：

$P(\text{success}|0)=\frac23$

$P(\text{success}|1)=\frac23$

$P(\text{success}|2)=\frac23$

于是：

$P_{\text{worst}}^{\text{qubit}}=\frac23$

也就是：

$66.7%$

8. Classical Bit 和 Qubit 的比较

Classical bit：

$P_{\text{worst}}^{\text{bit}}=\frac12=50%$

Qubit：

$P_{\text{worst}}^{\text{qubit}}=\frac23\approx66.7%$

所以：

$\frac23>\frac12$

这说明：

一个 qubit 虽然不能完美存储一个 trit，但在这个 communication task 中，可以比一个 classical bit 保留更多关于 trit 的信息。

9. 图中的紫色和红色方向

图中：

紫色：Alice 编码 trit 使用的三个 states
$|\phi_0\rangle,\ |\phi_1\rangle,\ |\phi_2\rangle$

红色：Bob 的三个测量方向
$|\phi_0^\perp\rangle,\ |\phi_1^\perp\rangle,\ |\phi_2^\perp\rangle$

可以理解为：

紫色 = candidate quantum states

红色 = measurement directions

它们不是同一个概念。

Bob 的任务是通过 measurement outcome 来推断 Alice 原来发送的是哪一个紫色状态。

如果你把书里的原图上传到 GitHub，可以在这里插入：

![Trine states and measurement directions](你的图片路径)

10. 书中给出的一个简单测量策略

书里先介绍了一个比较直观、但不是最优的 measurement strategy。

Bob 随机选择：

$k\in{0,1,2}$

然后使用正交基：

${|\phi_k\rangle,|\phi_k^\perp\rangle}$

进行测量。

也就是说，Bob 随机挑一个候选态 $|\phi_k\rangle$，然后测试：

收到的 qubit 更接近 $|\phi_k\rangle$，还是它的正交方向 $|\phi_k^\perp\rangle$？

11. Bob 根据测量结果怎么猜？

如果 measurement outcome 是：

$|\phi_k\rangle$

Bob 就猜：

$\ell=k$

也就是说：

Bob 认为 Alice 发送的就是第 $k$ 个 trit。

如果 measurement outcome 是：

$|\phi_k^\perp\rangle$

Bob 就知道：

原来的状态更可能不是 $|\phi_k\rangle$。

于是 Bob 从剩下两个 trit 中随机选择一个作为答案。

12. 这个简单测量策略的成功率

分成两种情况。

情况 1：Bob 随机选中的 $k$ 刚好等于真实 trit

这种情况发生概率：

$\frac13$

如果：

$k=\ell$

那么 Bob 正好用：

${|\phi_\ell\rangle,|\phi_\ell^\perp\rangle}$

去测量真实状态：

$|\phi_\ell\rangle$

所以一定得到：

$|\phi_\ell\rangle$

因此：

$P(\text{success}|k=\ell)=1$

这一部分对总成功率的贡献是：

$\frac13\times1$

情况 2：Bob 选中的 $k$ 不等于真实 trit

这种情况发生概率：

$\frac23$

即：

$k\neq\ell$

此时 Bob 用的是：

${|\phi_k\rangle,|\phi_k^\perp\rangle}$

去测量真实状态：

$|\phi_\ell\rangle$

由于 trine states 之间存在固定角度关系，测量得到：

$|\phi_k^\perp\rangle$

的概率是：

$\frac34$

如果得到：

$|\phi_k^\perp\rangle$

Bob 就知道原来的 trit 不是 $k$，于是只能在剩下两个 trit 中随机猜一个。

因此：

$P(\text{guess correctly}|\phi_k^\perp)=\frac12$

所以这一部分的成功概率贡献是：

$\frac23\times\frac34\times\frac12$

13. 总成功率

因此：

$P_{\text{success}}

\frac13(1)
+
\frac23\left(\frac34\right)\left(\frac12\right)$

计算：

$=
\frac13
+
\frac14$

所以：

$P_{\text{success}}

\frac7{12}$

即：

$P_{\text{success}}=\frac7{12}\approx58.3%$

14. 为什么 $\frac7{12}$ 已经说明 qubit 比 bit 强？

因为前面已经知道：

$P_{\text{worst}}^{\text{bit}}=\frac12=50%$

而这个简单的 qubit strategy：

$P_{\text{success}}=\frac7{12}\approx58.3%$

所以：

$\frac7{12}>\frac12$

也就是：

$58.3%>50%$

因此已经可以证明：

存在一种 qubit strategy，可以超过所有 classical bit strategies。

15. 但是 $\frac7{12}$ 还不是最优

书里紧接着说明：

$\frac7{12}$

还不是 qubit strategy 的最好结果。

还可以设计更好的 measurement，使成功率继续提高。

例如可以达到：

$\frac23\cos^2\left(\frac{\pi}{12}\right)$

甚至在更一般的 POVM measurement 下，可以进一步研究真正的最优 strategy。

因此这里的 $\frac7{12}$ 主要作用是：

先证明 qubit 确实可以 outperform classical bit。

16. 整个通信流程

整个问题可以理解成：

Alice 得到 trit 0/1/2

→ 编码成三个不同 qubit states 之一

→ 把 qubit 发送给 Bob

→ Bob 选择 measurement

→ 得到 measurement outcome

→ 根据 outcome 猜原来的 trit

17. 核心区别

Classical bit 只有两个可以完美区分的编码：

$0$

和：

$1$

但是 Alice 有三个输入：

$0,\ 1,\ 2$

所以一定需要丢失一部分信息。

Qubit 虽然也是二维系统，不能提供三个两两正交的状态，但是它可以利用：

non-orthogonal states

probability amplitudes

phase

quantum measurement

把三个 trit 更均匀地编码进一个 qubit。

因此在某些任务指标上：

$P_{\text{qubit}}>P_{\text{bit}}$

18. 一句话总结

一个 qubit 不能完美传递一个 trit，因为二维 Hilbert space 中不存在三个两两正交的量子态；但是 Alice 可以把三个 trit 编码成三个对称的 non-orthogonal qubit states，Bob 再设计合适的 measurement，从而使 worst-case success probability 超过 classical bit。

最重要的比较是：

$P_{\text{worst}}^{\text{bit}}=\frac12=50%$

而一个简单的 qubit strategy 已经可以做到：

$P_{\text{success}}=\frac7{12}\approx58.3%$

更好的 measurement 还可以继续提高成功率。

另外，对于对称 trine states：

\frac32I$

这个 $\frac32I$ 是三个 trine state projection matrices 实际相加得到的，不是人为定义的。

为了构造合法 POVM，需要满足：

$E_0+E_1+E_2=I$

所以定义：

$E_i=\frac23|\psi_i\rangle\langle\psi_i|$

因为：

$\frac23\times\frac32=1$

于是：

$\sum_iE_i=I$

最终：

$P(i|i)=\frac23$

所以对称 POVM 下每个状态的正确识别概率可以达到：

$\frac23=66.7%$
